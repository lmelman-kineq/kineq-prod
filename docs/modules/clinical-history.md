...
**Actualización (implementado) — navegación de secciones compacta en mobile**: en `≤820px`, `AssessmentSectionNav` (las 4 tabs principales — Antecedentes/Seguridad clínica/Contexto y Hábitos/Dolor y función) se reemplaza por `MobileSectionSelect` (`InitialAssessmentPanel.tsx`, `useIsMobile()`) — un `<select>` compacto de ancho completo, con el mismo chevron custom que ya usa el resto de Kineq (`.select-chevron-wrap`), en vez de cuatro tabs apretadas. Componente nuevo y genérico (`frontend/src/components/MobileSectionSelect.tsx`), reusado también en Configuración y Estadísticas (ver "Rediseño mobile-first" en `docs/modules/dashboard.md`) — un solo componente en vez de tres implementaciones casi iguales. En desktop, `AssessmentSectionNav` sigue igual, sin cambios.

**Actualización (implementado) — leyendas redundantes ocultas en mobile**: dos textos que solo restatean lo que el propio control ya comunica se ocultan en `≤820px` (siguen visibles en desktop): "Completá la información clínica base del paciente. Los cambios se guardan automáticamente." (se muestra solo antes de la primera ficha guardada; "Última actualización: ..." — que sí es información real, no solo relleno — se mantiene siempre) y "Texto libre complementario a la lista de arriba." (descripción de la sección "Notas adicionales"). El resto de las descripciones de sección (ej. "Opcional — completar solo si es clínicamente relevante.") se mantuvo: explican una regla no obvia (que esa sección no es obligatoria), no solo repiten el título.

**Actualización (implementado) — placeholder de alergias unificado**: "Buscar alergia conocida o escribir una nueva..." (`FichaAllergyList.tsx`) pasó a "Buscar o crear alergia" — más corto, mismo significado, funciona igual en desktop y mobile (no había necesidad de dos textos distintos).
...
**Actualización (implementado) — adjuntar archivo ya en el alta, "upload diferido"**: antes, crear un estudio nuevo mostraba el aviso "Guardá el estudio primero; después vas a poder adjuntarle un archivo" — un archivo solo se podía sumar en un segundo paso, editando el estudio ya guardado. Se sacó ese aviso y ese requisito: el formulario de alta (`FichaEstudiosList.tsx`) ahora tiene su propio selector "↑ Subir archivo(s)" (mismo estilo que el de Evolución), con el nombre del archivo elegido mostrado en un chip removible/reemplazable antes de guardar, validación de tipo/tamaño inline (mismos límites de arriba) y sin usar nunca `alert()` nativo. El archivo elegido se guarda en estado de React (`File`, nunca se sube todavía) — recién al tocar "Guardar" se ejecuta la secuencia **upload diferido**: 1) `POST` crea el `FichaEstudioComplementario` (sin archivos), 2) con el `id` real ya devuelto, sube los archivos (`onUploadArchivos`, mismo endpoint que ya existía para estudios existentes). Si el paso 1 falla, no hay nada que subir. Si el paso 2 falla, el estudio **ya quedó guardado** (no se pierde el alta por un error de red al subir los archivos) — se muestra un error explicando que no se pudieron subir y que se puede reintentar desde la fila recién creada en la lista, en vez de bloquear o revertir el alta. `useFichaInicial.ts`'s `addEstudio()` devuelve el estudio creado (con su `id` real), que es lo que permite encadenar el upload sin esperar a un segundo `refresh()`. El formulario de **edición** de un estudio existente no cambió — sigue usando la fila de la lista para adjuntar/quitar archivos.

### Estudios: múltiples archivos (implementado)

**Actualización (implementado)**: un Estudio pasó de soportar un único archivo a soportar varios (`Estudio → *N archivos`), pensado para el caso real de "radiografía frente + radiografía perfil + informe" bajo una misma entrada de estudio.

**Diagnóstico previo (por qué se necesitaba un modelo nuevo, no solo un endpoint nuevo)**: `FichaEstudioComplementario` guardaba el archivo como 4 columnas escalares directamente en la fila (`archivoPathname`/`archivoNombreOriginal`/`archivoMimeType`/`archivoSizeBytes`) — estructuralmente solo podía haber uno; subir un archivo nuevo *reemplazaba* al anterior (`estudioArchivoRoutes.ts`'s `/confirm` viejo borraba el blob anterior antes de guardar el nuevo). `EvolucionImagen` ya era el precedente correcto para "1 padre → N archivos" (tabla hija propia, FK, `onDelete: Cascade`, ruta `/upload-tokens` plural) — Estudio no tenía el equivalente.

**Modelo**: nueva tabla `EstudioArchivo` (migración `20260906120000_estudio_archivo_multiple`, ver `docs/database.md`), mismo shape que `EvolucionImagen`. Las 4 columnas viejas de `FichaEstudioComplementario` quedan deprecadas (nullable, ya no se leen ni escriben) — un backfill en la propia migración copia cualquier archivo histórico a una fila de `EstudioArchivo`, así que un estudio viejo con un archivo pre-migración sigue funcionando exactamente igual (mismo criterio de compatibilidad que el resto de las migraciones de este proyecto — nunca se pierde una URL/referencia existente).

**Backend** (`estudioArchivoRoutes.ts`, reescrito de cero sobre el patrón de `evolucionImagenesRoutes.ts`): `POST .../archivos/upload-tokens` (plural, hasta 10 archivos por estudio, valida cada uno — tipo/tamaño — antes de emitir ningún token; si uno falla, no se emite ninguno de la tanda), `POST .../archivos/confirm` (crea todas las filas de `EstudioArchivo` de la tanda dentro de una `$transaction` — o se guardan todos los items de esa tanda o ninguno, para que el usuario nunca crea que se adjuntaron todos si solo entraron algunos), `GET .../archivos/:archivoId/contenido` (sirve un archivo puntual), `DELETE .../archivos/:archivoId` (borra un archivo puntual, sin afectar a los demás del mismo estudio). Mismo mecanismo de Vercel Blob privado de siempre (presigned URL, upload directo navegador→Blob, nunca pasa por esta función) — ver "Archivos: Vercel Blob" en `docs/architecture.md`. Mismos permisos que antes: cualquier `CLINICAL_ROLES` con profesional vinculado, sin restricción de "solo el propio autor".

**Frontend** (`FichaEstudiosList.tsx`): el `<input type="file">` (tanto el de "adjuntar al crear" como el de "adjuntar a uno ya existente") ahora tiene `multiple` — al elegir varios archivos de una vez, cada uno se valida individualmente (tipo/tamaño) y se agregan a una lista de staged files (o se suben directo, si el estudio ya existe), mostrados uno por fila (nunca uno al lado del otro forzando scroll horizontal — `overflow-wrap: anywhere`, pensado para nombres de archivo largos). Cada archivo listado (staged o ya subido) tiene su propia acción "Quitar"/"Eliminar" independiente — quitar uno antes de guardar no afecta a los demás. En el detalle de un estudio, todos sus archivos se listan con su propia acción "Ver"/"Eliminar" (`openAuthorizedFile()`, mismo mecanismo de siempre — nunca la URL de Blob cruda).

**Qué no cambió**: el resto del formulario de Estudio (tipo, fecha, resumen, observaciones), el aislamiento por consultorio (cada `EstudioArchivo` valida `estudioId` + `consultorioId` igual que el resto de la app), y el modelo de permisos.
...
**Actualización (implementado) — bug real corregido: popover de "Resumen" quedaba colgado en mobile, duplicando el contenido de la card expandida**: `EvolutionTable.tsx` muestra un popover con el resumen completo al hacer `mouseenter`/`focus` sobre la fila (pensado para el resumen truncado con "..." de desktop). En touch no existe un `mouseleave` real — un tap dispara `mouseenter`+`focus` sintéticos sin su salida correspondiente — así que el popover quedaba abierto, flotando sobre la card que el mismo tap acababa de expandir (`toggleExpand`), mostrando el mismo resumen dos veces a la vez. Además, en mobile el resumen ya no se trunca (`@media 820px` lo muestra completo, `white-space: normal`), así que el popover no cumplía ningún propósito ahí — puro riesgo de bug. Fix: `showResumenPopover` (`useIsMobile()`) no hace nada en mobile — el popover nunca se dispara ahí, la única forma de ver el contenido completo es expandir la card (ya existente, sin cambios). Sin cambios en desktop.

**Nota sobre el resto de esta sección (imágenes adjuntas/upload) — desactualizada en el detalle mecánico, `docs/architecture.md` es la fuente de verdad actual**: describe una implementación anterior (`multer`/`memoryStorage`, un único `POST` de imágenes, `BLOB_READ_WRITE_TOKEN` estático) superada primero por el upload directo cliente→Blob con token de cliente, y luego por la reescritura a OIDC (`issueSignedToken`/`presignUrl`, sin ningún token estático) — ver "Archivos: Vercel Blob, upload directo cliente → Blob con OIDC (implementado)" en `docs/architecture.md` para el mecanismo real vigente. Los límites (5 imágenes, 10MB c/u, JPG/PNG/WEBP) y el modelo `EvolucionImagen` sí siguen vigentes tal cual se describen abajo.

**Actualización (implementado) — imágenes adjuntas**: una evolución puede tener hasta **5 imágenes** (JPG/PNG/WEBP, máximo 10MB cada una). El binario nunca se guarda en MySQL: se sube a **Vercel Blob** (`access: 'private'`) y en la base solo queda la referencia (`EvolucionImagen`: `pathname`, `nombreOriginal`, `mimeType`, `sizeBytes`, más `consultorioId`/`pacienteId`/`evolucionId` para poder filtrar y validar — **sin** columna `url`). El endpoint de subida (`POST /api/evoluciones/:evolucionId/imagenes`, `multer` con `memoryStorage`, campo `imagenes`) exige una evolución **ya creada** — por eso, al cargar una evolución nueva con imágenes, el frontend (`PatientDetailPage.tsx`) primero crea la evolución (`POST /api/evoluciones`) y recién después sube las imágenes seleccionadas a ese id (mientras tanto, las imágenes elegidas quedan solo en memoria del navegador — "sacar antes de guardar" es sacarlas de ese array local, sin ningún request). Mismo criterio de permisos que editar/eliminar la evolución: el profesional solo agrega o saca imágenes de sus propias evoluciones, administrador de cualquiera del consultorio; el backend siempre revalida `consultorioId`/dueño, nunca confía en lo que mande el cliente.
