...
**Actualización (implementado) — pantalla única de Paciente (con o sin turno activo)**: `AttentionPage.tsx` fue eliminado — `PatientDetailPage.tsx` es ahora el único componente para ver/atender un paciente, tanto entrando desde Pacientes/Turnos/Inicio como desde "Iniciar atención"/"Continuar atención". Un turno activo (`activeTurno`) solo agrega información/acciones de sesión (timer, "Finalizar atención", vínculo automático evolución↔turno) de forma aditiva, nunca reemplaza ni oculta funcionalidad propia del paciente: "Editar paciente" (`PatientFormModal.tsx`, mismos permisos `ADMINISTRADOR`/`RECEPCION` de siempre) y el selector de Diagnóstico dependen únicamente de permisos, nunca del estado del turno. El orden de tabs (Ficha inicial → Evoluciones → Turnos → Estudios), la edad junto a la fecha de nacimiento y el layout del Resumen clínico son idénticos en todos los contextos porque es literalmente la misma pantalla.

### Nombre completo

**Actualización (implementado) — un solo campo "Nombre completo"**: `PatientFormModal.tsx` (alta y edición) y el alta rápida desde Crear Turno (`FormFields.tsx`) ya no piden Nombre y Apellido por separado — piden un único campo "Nombre completo". **Decisión de schema**: no se agregó ningún campo `nombreCompleto` ni migración — el schema de `Paciente` sigue teniendo `nombre`/`apellido` (evita romper compatibilidad con datos y código existentes que ya asumían ambas columnas non-null). El valor ingresado se guarda completo en `nombre` y `apellido` queda `''`. Nunca se hace un parseo ingenuo (primera palabra = nombre, resto = apellido): eso pierde información en nombres reales como "María de los Ángeles Pérez". Un único helper, `frontend/src/utils/patient.ts` (`patientFullName()`), arma el nombre para mostrar en toda la app (`${nombre} ${apellido}`.trim()) — funciona igual para pacientes viejos (con apellido real) y nuevos (todo en `nombre`). Al editar un paciente viejo, el formulario precarga el nombre completo ya combinado (vía `patientFullName()`) y, al guardar, ese paciente también migra a "todo en `nombre`" — sin acción explícita de migración masiva, cada edición futura normaliza ese registro. `PatientAvatar.tsx` (iniciales del avatar) se ajustó para tomar primera+última palabra de `nombre` cuando `apellido` está vacío. Backend: `POST`/`PATCH /api/pacientes` ahora solo exigen `nombre` (ya no `apellido`); el listado (`GET /api/pacientes`) ordena por `nombre` en vez de `apellido, nombre`.

**Regla de UI de personas — extendida a Profesional y Usuario**: este mismo criterio (un único "Nombre completo", `apellido: ''`, sin parseo, sin migración) se extendió a `ProfesionalFormModal.tsx` (ver `docs/modules/professionals.md`) y a `UsuarioFormModal.tsx`/`RegisterPage.tsx` (ver `docs/modules/users-and-roles.md`). Es una regla de producto permanente, no solo de esta ronda: **cualquier alta/edición nueva que pida el nombre de una persona usa un único campo "Nombre completo" por default**, salvo una necesidad técnica/productiva explícita y documentada en contrario.

### Campos obligatorios iniciales

**Actualización (implementado)**: el único campo obligatorio para crear un paciente es **Nombre completo**, tanto en frontend (`PatientFormModal.tsx`) como en backend (`POST`/`PATCH /api/pacientes`, `backend/src/app.ts`). Documento, fecha de nacimiento, teléfono, email, dirección, obra social, número de afiliado y observaciones son opcionales — el schema de Prisma (`Paciente`) ya los tenía como columnas nullable, así que no hizo falta ninguna migración para volverlos opcionales, solo sacar la validación extra que el frontend agregaba únicamente en el alta (`isCreate`). Un campo opcional vacío se guarda como `null`, nunca como `''` (ver "Documento / DNI" abajo). Esto también habilita el alta rápida de paciente desde Crear Turno (ver `docs/modules/appointments.md`), que solo pide Nombre completo como obligatorio (más Documento/Teléfono opcionales).
...
**Actualización (implementado, parcial) — imágenes en Evoluciones**: la pregunta de "a qué se adjuntan los documentos" ya tiene una primera respuesta concreta para el caso de fotos clínicas: se pueden adjuntar imágenes (no otros tipos de archivo) directamente a una Evolución — ver "Imágenes en Evoluciones" en `docs/modules/clinical-history.md`. Documentos a nivel paciente/turno/tratamiento en general siguen sin implementar.
...
**Actualización (implementado) — foto real del paciente**: el placeholder "La carga de imágenes estará disponible próximamente" del avatar (`PatientProfileHeader.tsx`) ya no existe — el lápiz abre un popover real (subir/reemplazar/quitar) que sube a **Vercel Blob privado** (`Paciente.fotoPathname`/`fotoMimeType`, `POST`/`DELETE /api/pacientes/:id/foto`, JPG/PNG/WEBP hasta 5MB). Mismo criterio de permisos que el resto de datos administrativos del paciente (`ADMIN_DATA_ROLES`, los cuatro roles); en la UI, el botón de foto se muestra junto con "Editar paciente" (gateado por `canEditAdmin` en `PatientDetailPage.tsx`, no exactamente los mismos cuatro roles del backend — ver nota en esa pantalla). Sin foto, sigue mostrando `PatientAvatar.tsx` (iniciales) como fallback; con foto, `AuthorizedImg.tsx` la pide autenticada (nunca una URL pública) y arma un object URL local. Ver "Archivos: Vercel Blob (implementado)" en `docs/architecture.md` para el mecanismo compartido con Evoluciones/Estudios/foto de Usuario.
...
**Actualización (implementado) — reorganización de la pantalla del Paciente: sin card lateral, "Resumen clínico" como primera tab**: `PatientDetailPage.tsx` tenía un grid de dos columnas (`.patient-detail-layout`, eliminado) con el `ClinicalSummaryPanel` en una card lateral fija y las tabs clínicas (Ficha inicial/Evoluciones/Turnos/Estudios) en la card principal. Ahora la card principal ocupa todo el ancho — no hay card lateral — y el mismo `ClinicalSummaryPanel` (sin cambios de contenido: alertas, antecedentes relevantes, estado de ficha, última evolución, próximo turno) pasó a ser una tab más, la **primera**: **Resumen clínico → Ficha inicial → Evoluciones → Turnos → Estudios** (gateada por `canEditClinical`, igual que las demás tabs clínicas — para roles sin acceso clínico sigue sin aparecer, solo ven Turnos). Es la tab que abre por default para roles con acceso clínico (antes era Ficha inicial). CSS eliminado por quedar sin uso: `.patient-detail-layout`, `.patient-detail-layout--full`, `.patient-detail-stack`, y el `order`/`order` que interlazaba ambas columnas.

**Actualización (implementado) — "Resumen clínico" en dos columnas**: `.clinical-summary` (dentro de `ClinicalSummaryPanel.tsx`) pasó de una sola columna a `grid-template-columns: repeat(2, minmax(0, 1fr))`. Las alertas, el bloque de Antecedentes (`.clinical-summary-antecedentes`) y el link "Ir a ficha inicial →" ocupan las dos columnas (contenido de longitud variable, no se prestan bien a una mitad fija); "Estado de ficha inicial", "Última evolución" y "Próximo turno" se acomodan de a dos por fila. Colapsa a una sola columna en mobile (≤820px, mismo breakpoint que el resto de la app).

**Las dos actualizaciones de arriba (sin card lateral + Resumen clínico como primera tab en dos columnas) quedaron completamente reemplazadas por una ronda posterior — no reflejan el estado actual.** Layout actual, de más reciente a más vieja:

1. **Rediseño 70/30 (ronda "UX compacta, Evoluciones, permisos y rediseño de Paciente")**: `.patient-detail-layout` volvió a existir, pero como un grid `7fr 3fr` — panel principal de tabs clínicas a la izquierda (~70%), columna angosta a la derecha (~30%) con dos cards apiladas: `PatientProfileHeader` (datos del paciente, ahora vertical) arriba, `ClinicalSummaryPanel` abajo. **Resumen clínico dejó de ser una tab** — vive como card fija en la columna derecha, siempre visible sin cambiar de tab. Su contenido también cambió: ya no muestra "Estado de ficha inicial"/"Última evolución"/"Próximo turno" ni el bloque de Antecedentes agrupados — quedó reducido a únicamente alertas clínicas (mismo cálculo que `PatientAlertsBadge`/el viejo popup de detalle), con "Ficha inicial pendiente" como una alerta más. `activeTurno` (contexto de "Iniciar/Continuar atención") ya no agrega ninguna card visual — la vieja franja de estado/fecha·hora/duración reservada se eliminó; solo queda el botón "Finalizar atención" en la topbar.
2. **Ajustes finos posteriores**: se sacó el badge/contador de alertas de la card superior (redundante con la card de abajo, que ya muestra el detalle completo) — `PatientAlertsBadge.tsx` y el popup `ClinicalAlertsDetail.tsx` que abría quedaron sin uso y se borraron. El badge "Activo" bajo el nombre solo se muestra cuando el paciente NO está activo (antes se mostraba siempre). El botón de editar de la topbar y el texto "Volver a Pacientes" se redujeron de tamaño.
3. **Orden de tabs (ronda "Plantillas de Evolución, tabs y ajustes de Ficha Inicial")**: **Evoluciones → Ficha inicial → Estudios → Turnos** (antes Ficha inicial → Evoluciones → Turnos → Estudios). Evoluciones es la tab que abre por default para roles con acceso clínico (antes era Ficha inicial / Resumen clínico, según la ronda).
