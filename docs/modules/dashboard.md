...
**Botón "Hoy" en mobile cambia a Día** (`goToToday`, `App.tsx`): además de volver a la fecha actual, en mobile (`isMobile`) también fuerza `calendarView` a `'day'` — pedido explícito ("Hoy siempre debe mostrar el calendario diario de hoy"). En desktop, sin cambios: mantiene la vista activa, como siempre.
