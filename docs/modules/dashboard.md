...
Grilla de 7×4-6 semanas (siempre semanas completas, incluye días de meses adyacentes atenuados). Cada celda muestra el número de día y hasta 3 turnos como chips compactos (color de especialidad + horario si entra); si hay más, se muestra "+N más". Click en un turno-chip (o "+N más") navega a Semana con ese día seleccionado, en cualquier plataforma (ver Ronda 3 — reemplaza la navegación directa a Día de una ronda anterior). Click en un slot de un día vacío abre alta rápida con esa fecha; como Mes no tiene una hora asociada a la celda, no se inventó un horario arbitrario — la hora la completa el usuario en el formulario, igual que "Más opciones" ya lo permite hoy. Mobile: label compacto centrado arriba de la grilla ("Sep 2026", `.calendar-period-compact-label`) reemplaza el título largo redundante junto a las flechas.
...
**Mes → Semana ahora también en desktop** (`MonthView`'s `onSelectDay`, `App.tsx`): se quitó el `isMobile ? 'week' : 'day'` de la ronda anterior — tocar un turno-chip (o "+N más") en Mes siempre lleva a Semana con ese día seleccionado, en cualquier plataforma. Reemplaza la decisión mobile-only de la Ronda 2 (pedido explícito de esta ronda, no una corrección de un error). Tocar una celda vacía sigue abriendo alta rápida si el rol puede crear turnos, sin cambios.
