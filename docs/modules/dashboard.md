...
- **Navegación**: las flechas prev/next (`goToPreviousPeriod`/`goToNextPeriod`) respetan la vista activa vía `navigateDate(selectedDate, calendarView, ±1)` — Día ±1 día, Semana ±7 días, Mes ±1 mes calendario (con clamp de fin de mes), Año ±1 año. El botón "Hoy" (`goToToday`) vuelve al período que contiene la fecha actual **y siempre fuerza la vista a Día**, en desktop y mobile por igual (ver Ronda 5 más abajo — antes, en desktop, mantenía la vista activa; se unificó a pedido explícito).
- **Mes → Semana / Año → Mes** (ambas plataformas, ver Ronda 3 más abajo): tocar un turno en Mes navega a Semana con ese día seleccionado. Tocar la tarjeta de un mes en Año navega a Mes con ese mes activo, preservando `selectedDate` si ya pertenece a ese mes o usando el día 1 si no.
...
Grilla de 7×4-6 semanas (siempre semanas completas, incluye días de meses adyacentes atenuados). Cada celda muestra el número de día y hasta 3 turnos como chips compactos (color de especialidad + horario si entra); si hay más, se muestra "+N más". Click en cualquier parte de la celda de un día (no solo el turno-chip) navega a Semana con ese día seleccionado, en cualquier plataforma (ver Ronda 3 — reemplaza la navegación directa a Día de una ronda anterior). Mes es una vista puramente de navegación: no abre alta rápida desde ninguna celda (ver Ronda 4 — antes sí lo hacía para roles que pueden crear turnos, pisando accidentalmente el gesto de navegar a Semana; se quitó esa rama por completo). Para crear un turno en una fecha de Mes: navegar a Semana/Día con ese día ya seleccionado y usar el slot horario, el botón "Nuevo turno" o el FAB. Mobile: label compacto centrado arriba de la grilla ("Sep 2026", `.calendar-period-compact-label`) reemplaza el título largo redundante junto a las flechas.

### Año

12 mini-calendarios (Enero-Diciembre), puramente de navegación — sin datos operativos de turnos (sin fetch de turnos para esta vista, per "no bajar datos clínicos innecesarios" cuando no hay nada que mostrar con ellos). Click en un número de día navega a Día de esa fecha (`stopPropagation` — no dispara también la navegación de la tarjeta). Click en cualquier parte de la tarjeta de un mes (no solo el nombre — toda `.year-view-month` es clickeable, con `role="button"`/`tabIndex`/`onKeyDown`/`aria-label` para teclado, ver Ronda 3) navega a Mes con ese mes activo, preservando `selectedDate` si ya pertenece a ese mes o usando el día 1 si no. Los indicadores de turno (punto) eran explícitamente opcionales en la spec y no se implementaron — prioridad 1 (navegación correcta) y 2 (12 meses) sobre esa mejora cosmética. Responsive: 4 columnas en desktop, 2 en tablet (`≤820px`), 1 en mobile (`≤480px`) — verificado sin overflow horizontal en 390px. Mobile: label compacto centrado arriba de la grilla ("2026", `.calendar-period-compact-label`) reemplaza el título largo redundante junto a las flechas.
...
### Ajustes de seguimiento (feedback tras la primera pasada)

- **"No hay turnos para este día" eliminado** (Día, desktop y mobile por igual): el `<p className="empty-day-message">` se sacó del todo — no se reemplazó por nada, un día vacío ahora solo muestra la grilla horaria en blanco. `.empty-day-message` se borró de `App.css` (sin otros usos).
- **Gap entre "Turnos del día" y los controles de navegación, mobile**: el causante real no era el `margin` de `.main-header` (que sí se había tocado en la primera pasada, sin efecto) sino el `gap: 24px` del contenedor flex `.main-area` — se angostó a `8px` solo en mobile.
- **"Calendario del día"/"Calendario semanal"/etc. (`<h2>`) oculto en mobile, en las 4 vistas** — es redundante con el selector de vista, que ya dice qué vista es.
- **La fecha larga (`<p>`, ej. "viernes 4 de septiembre de 2026") oculta en mobile — pero solo en Día y Semana**, no en Mes/Año: Semana ya tiene el equivalente en su franja de 7 días, y Día pasa a mostrar el pill descripto abajo; Mes/Año no tienen ningún otro indicador de qué mes/año están mostrando, así que conservan su período corto (ej. "Septiembre 2026", "2026") — no es la "fecha completa" que se pidió sacar, y sacarla ahí dejaría la vista sin ningún contexto de período en mobile. (Alcance no 100% explícito en el pedido — fácil de revertir con una línea de CSS si se prefiere ocultarlo también ahí.)
- **Header "VIE / 4 sep" para Día, mobile**: mismo bloque visual que ya usaba el header de columna de Semana (`.week-day-header`, con el mismo modificador de pill morado — nuevo `--active`, ver abajo), ahora también arriba del timeline de Día, en la misma grilla de 2 columnas (`44px` + resto) que `.day-grid`, alineado con el eje horario de abajo. Reemplaza al `<h2>`/`<p>` que se ocultan ahí.
  - `.week-day-header--today` (Semana) marca literalmente "hoy" — pero el header de Día siempre muestra el único día visible, sea o no hoy. Se agregó `.week-day-header--active` como modificador aparte (misma regla CSS que `--today`, solo para el pill) en vez de reusar `--today` con un significado distinto — Día siempre lo aplica, Semana sigue usando `--today` para su sentido original.
...
### Ronda 2 — swipe, bug real del header del sheet, Hoy/Mes mobile, header compacto

**Swipe horizontal para navegar el calendario** (nuevo `frontend/src/hooks/useSwipeNavigation.ts`, primer uso de eventos táctiles en el proyecto — confirmado que no existía ninguno antes): `onTouchStart`/`onTouchEnd` en un contenedor nuevo (`.calendar-swipe-area`, envuelve el cuerpo del calendario, cualquier vista) — umbral de 50px horizontal, descarta el gesto si el desplazamiento vertical supera 60px (scroll, no swipe) o si arranca a menos de 24px del borde izquierdo (compite con el swipe-back nativo de iOS). Reutiliza `goToNextPeriod`/`goToPreviousPeriod` (los mismos handlers de las flechas, ver más abajo) — izquierda = siguiente período, derecha = anterior, respetando la vista activa igual que las flechas. Al vivir solo dentro de `.calendar-swipe-area`, nunca compite con bottom sheets/dropdowns/scroll de formularios, que están fuera de ese contenedor.

**Bug real corregido: contenido del formulario tapaba "Guardar" del header sticky al hacer scroll** — diagnosticado y reproducido en vivo (no solo a partir de la captura del usuario): `.modal-header` es `position: sticky`, que en Chromium **siempre** crea su propio stacking context (con o sin `z-index` explícito — mismo motivo, ya documentado, del bug del popover de la sidebar). Sin un `z-index` propio, un descendiente posicionado del formulario que aparece *después* en el DOM (ej. `.date-input-picker-button`, `position: absolute`) podía pintarse por encima del header sticky durante el scroll, tapando "Guardar"/el ícono. Fix: `z-index: 2` explícito en `.modal-header` (mobile). Verificado con `document.elementFromPoint` antes/después del fix, en varios puntos de scroll — un "sliver" de contenido pasando brevemente por encima del área del header durante el scroll activo (antes de asentarse detrás) es comportamiento normal de cualquier header sticky y no es lo que se corrigió; lo corregido es que el contenido quedara **atascado** encima del header, no la transición momentánea.

**Botón "Hoy" en mobile cambia a Día** (`goToToday`, `App.tsx`): además de volver a la fecha actual, en mobile (`isMobile`) también fuerza `calendarView` a `'day'` — pedido explícito ("Hoy siempre debe mostrar el calendario diario de hoy"). En desktop, sin cambios: mantiene la vista activa, como siempre. **Superado en la Ronda 5** (más abajo): ahora el mismo comportamiento aplica también en desktop.

**Mes → Semana en mobile, tocar un turno-chip** (`MonthView`'s `onSelectDay`, llamado únicamente al tocar un turno o "+N más" — tocar una celda vacía sigue abriendo alta rápida si el rol puede crear turnos, sin cambios): en mobile pasa a `setCalendarView('week')` en vez de `'day'`. En desktop, sin cambios (sigue yendo a Día, decisión deliberada de una ronda anterior). Año → Día (tocar un día) no cambió en ninguna plataforma.

**Header de navegación compacto en mobile** (`.calendar-nav`): `justify-content: center` + `.calendar-nav-today` más angosto — agrupa "Hoy [‹] [›] período" centrado en vez de quedar pegado a la izquierda con huecos.

### Validación (ronda 2)

`tsc -b --force`/`vitest run` (168/168, sin tests nuevos de swipe — el hook es una utilidad de gestos sin lógica de negocio propia más allá de reusar `navigateDate`, ya cubierto)/`build`/`lint` limpios. Con Playwright (`hasTouch: true`, `TouchEvent` sintéticos reales — `page.mouse` no dispara touch events, necesarios para los handlers `onTouchStart`/`onTouchEnd`): swipe izquierda avanza el período (confirmado cambio de "Dom 6 sep" a "Lun 7 sep"), Hoy desde Mes vuelve a Día, tocar un turno-chip en Mes navega a Semana. Regresión de desktop explícita (Configuración/Estadísticas, ver más abajo) sin cambios.

### Ronda 3 — Mes → Semana y Año → Mes en ambas plataformas, labels compactos mobile

**Mes → Semana ahora también en desktop** (`MonthView`'s `onSelectDay`, `App.tsx`): se quitó el `isMobile ? 'week' : 'day'` de la ronda anterior — tocar un turno-chip (o "+N más") en Mes siempre lleva a Semana con ese día seleccionado, en cualquier plataforma. Reemplaza la decisión mobile-only de la Ronda 2 (pedido explícito de esta ronda, no una corrección de un error). Tocar una celda vacía seguía abriendo alta rápida si el rol podía crear turnos, sin cambios **en esta ronda** — corregido en la Ronda 4 (ver más abajo), que quitó esa rama por completo.

**Año → Mes, ambas plataformas, con regla de fecha ancla** (`YearView`'s `onSelectMonth`, `App.tsx`): tocar un mes en Año navega a Mes con ese mes activo. Regla: si `selectedDate` ya pertenece al mes tocado, se mantiene tal cual (día preservado); si no, se usa el día 1 del mes tocado. Nunca se usa la fecha actual si pertenece a otro mes. Año → Día (tocar un día dentro de una tarjeta de mes) no cambió.

**Toda la tarjeta de mes en Año es clickeable** (`.year-view-month`), no solo el nombre del mes — con `role="button"`, `tabIndex`, `onKeyDown` (Enter/Espacio) y `aria-label` (ej. "Abrir noviembre de 2026") para accesibilidad de teclado. Los botones de día dentro de la tarjeta (`.year-view-day`) llaman `event.stopPropagation()` para que tocar un día siga yendo a Día sin disparar también la navegación Año → Mes de la tarjeta.

**Label compacto centrado, mobile, Mes/Año** (nuevo `.calendar-period-compact-label`): reemplaza el título largo (`<h2>`/`<p>` de `.calendar-nav-title`, ej. "Calendario mensual" / "septiembre de 2026") que quedaba redundante junto a las flechas. Mes muestra "Sep 2026" (mes corto + año, reusa `MONTH_NAMES_SHORT`), Año muestra "2026". Día y Semana no lo necesitan — ya tienen su propio label (el pill "VIE / 4 sep" de Día, y la franja de 7 días de Semana), sin cambios ahí.

**`.calendar-nav-title` ahora oculto en mobile para las 4 vistas** (antes solo para Día/Semana; Mes/Año lo mostraban por no tener todavía un label propio). En desktop sigue visible en las 4 vistas, sin cambios.

**Explícitamente sin cambios**: comportamiento de "Hoy" (`goToToday`, incluido el hecho de que en desktop no cambia `calendarView`), flechas (`goToPreviousPeriod`/`goToNextPeriod`), swipe (`useSwipeNavigation`), header/sidebar/filtros/alta rápida/modo oscuro.

### Validación (ronda 3)

`tsc -b --force`/`vitest run` (168/168)/`build`/`lint` limpios (sin cambios de backend). Con Playwright, desktop (1400×900) y mobile (390×844, `hasTouch: true`): Mes → tocar turno-chip → Semana con el día de hoy resaltado en ambas plataformas; Año → tocar tarjeta de un mes distinto → Mes activo con el día 1 de ese mes (label "Nov 2026"); Año → tocar la tarjeta del mes **actual** (mismo mes que `selectedDate`) → Mes activo con el día original preservado (verificado llevando el resultado a Semana y confirmando que el día seleccionado en la franja mobile sigue siendo "6", no "1"); Año → tocar un día dentro de una tarjeta → Día, sin navegación duplicada (`stopPropagation` funcionando); labels compactos "Sep 2026"/"2026" presentes solo en mobile y visibles en modo oscuro; `.calendar-nav-title` oculto en mobile en las 4 vistas, visible en desktop; regresión explícita de Hoy (desktop se queda en la vista activa, mobile fuerza Día — ambos sin cambios), flechas (avanza un día) y swipe (`TouchEvent` sintéticos, avanza un día) en Día mobile, todo sin cambios respecto de rondas anteriores.

### Ronda 4 — Bug real: Mes/Año abrían "Nuevo turno" en vez de navegar; alineación de "Nro. de sesión" en Listado de turnos

**Causa real del bug**: `MonthView`'s celda de día (`.month-view-cell`) tenía `onClick={() => (canCreate ? onCreateSlot(date) : onSelectDay(date))}` — para cualquier rol que puede crear turnos (Administrador/Recepción/Supervisor, la mayoría de los usuarios reales), tocar la celda en **cualquier punto que no fuera exactamente un turno-chip o "+N más"** (que sí llaman `onSelectDay` con `stopPropagation`) abría "Nuevo turno" con `onCreateSlot(date)`. Esto incluía tocar el número de día, el espacio vacío de una celda con turnos, o cualquier celda sin turnos — exactamente el gesto que un usuario esperaría que navegue a Semana (Ronda 3). No era un bug nuevo introducido por la Ronda 3: esa rama `canCreate ? onCreateSlot : onSelectDay` ya existía antes y era consciente/deliberada en su momento (permitía crear un turno tocando un día en Mes) — lo que cambió es que ahora Mes tiene una interacción de navegación propia (Ronda 3) que ese mismo tap termina pisando para la mayoría de los roles. `YearView` no tenía ningún camino hacia `onCreateSlot` (ya reescrita en la Ronda 3 para navegar únicamente) — no se encontró ahí una causa de código equivalente; se verificó explícitamente con Playwright que tocar una tarjeta de mes en Año nunca abre "Nuevo turno", en desktop y mobile.

**Corrección**: se eliminó por completo la rama de creación de `MonthView` — `onCreateSlot`/`canCreate` salieron de `MonthViewProps` y del componente; la celda de día ahora siempre llama `onSelectDay(date)` (click y `Enter`/`Espacio`), sin condicional de rol. Mes pasa a ser una vista de navegación pura (como ya lo era Año): la creación rápida queda limitada a los contextos con intención explícita de agendar — botón "Nuevo turno" del header, FAB (mobile), slot horario vacío en Día, slot horario vacío en Semana — ninguno de los cuales se tocó. No fue necesario ningún `stopPropagation()` nuevo ni ningún guard por vista: al no quedar ningún handler de creación en Mes, no hay nada que "ganarle" por bubbling.

**Bug real #2 — "Nro. de sesión" en "Listado de turnos" (`TurnosPage.tsx`), mobile**: mismo defecto que ya se había corregido en la tabla de Turnos del Paciente (ver `docs/modules/patients.md`), pero en la tabla del listado principal. `.turnos-table-col-sesion` fija `width: 1%` en desktop (angostar la columna "Sesión" a lo mínimo) — en mobile, donde cada `<td>` es su propia fila `display: flex`, ese `width: 1%` encogía la fila entera (label + valor) contra el borde izquierdo de la card en vez de dejar que `justify-content: space-between` empujara el valor al borde derecho, como el resto de las filas. Fix: `width: auto` en el breakpoint mobile (mismo patrón que `.patient-turnos-table th/td`).

**Explícitamente sin tocar**: diseño general del calendario, backend/Prisma, recurrencia, filtros, permisos, timezone, diseño completo de las cards, datos de `numeroSesion`.

**Bug real #3 — label compacto de Mes/Año visible también en desktop**: `.calendar-period-compact-label` (Ronda 3) solo tenía estilos dentro de `@media (max-width: 820px)` — sin ninguna regla que lo ocultara fuera de ese breakpoint, un `<p>` es `display: block` por defecto del navegador, así que en desktop se mostraba igual (duplicando la fecha larga que ya muestra `.calendar-nav-title` debajo de "Calendario mensual"/"Calendario anual"). Fix: se sumó a la lista de elementos "ocultos por defecto, mostrados solo en mobile" que ya usa el proyecto (`.fab-new-turno, .modal-header-save-mobile, .sheet-drag-handle, .week-day-strip, .day-grid-mobile-header` — un único bloque `display: none` fuera de la media query, cada uno reactivado adentro) — mismo patrón, sin inventar uno nuevo.

### Validación (ronda 4)

`tsc -b --force`/`vitest run` (168/168)/`build`/`lint` limpios (sin cambios de backend). Con Playwright, desktop (1400×900) y mobile (390×844, `hasTouch: true`), usuario Administrador (`puedeCrearTurnos = true`, el caso que reproducía el bug): Mes → tocar una celda vacía → Semana, sin abrir "Nuevo turno" (antes: sí lo abría); Mes → tocar el número de día de una celda **con** un turno (sin tocar el chip) → Semana, sin modal; Año → tocar el cuerpo de una tarjeta de mes → Mes, sin modal, en ambas plataformas. Regresión explícita: Día → tocar slot vacío → sí abre "Nuevo turno"; Semana → tocar slot vacío → sí abre "Nuevo turno"; botón "Nuevo turno" del header → sí abre el modal; FAB mobile → sí abre el modal — los cuatro caminos válidos de creación intactos. "Listado de turnos" mobile: celda "Sesión" verificada con valor de un dígito y de dos dígitos (`12`), confirmado que el borde derecho del valor queda a la misma distancia del borde de la card que el resto de las filas (antes: apretado contra el label, a la izquierda).

### Ronda 5 — "Hoy" en desktop ahora fuerza Día (unificado con mobile)

**Cambio de comportamiento explícito** (`goToToday`, `App.tsx`): antes, en desktop, "Hoy" solo movía `selectedDate` sin tocar `calendarView` (si estabas en Mes, seguías en Mes). Ahora fuerza `calendarView` a `'day'` siempre, igual que ya hacía en mobile desde la Ronda 2 — se quitó el `if (isMobile)` que hacía esa rama condicional. Pedido explícito del usuario, no una corrección de bug.
