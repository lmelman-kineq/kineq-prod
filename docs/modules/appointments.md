...
**Bug real corregido — menú contextual vacío (rectángulo blanco) en Semana, para turnos de un día distinto al seleccionado**: el render del menú (`App.tsx`) busca el turno del click (`menuTurno`) para decidir qué mostrar — pero buscaba únicamente en `turnosState`, el estado que solo trae los turnos del **día actualmente seleccionado** (`selectedDate`). En Semana (y Mes), los bloques de turno vienen de `rangeTurnosState` (el rango completo visible, ver arquitectura de doble estado más abajo) — un turno de cualquier día de la semana *distinto* al seleccionado no existía en `turnosState`, así que `menuTurno` quedaba `undefined` y, como cada ítem del menú está condicionado a `menuTurno` (incluido "Ver Historia Clínica", que nunca debería depender de permisos), no se renderizaba ninguno — el menú quedaba como un rectángulo vacío. Fix: la búsqueda ahora mira `turnosState` primero y, si no lo encuentra ahí, `rangeTurnosState` — un solo punto de fix ya que Día y Semana/Mes comparten el mismo bloque de render del menú. Verificado con Playwright: click derecho sobre un turno de otro día de la semana (no el `selectedDate` actual) en vista Semana → menú completo (Ver Historia Clínica, Editar Turno, acciones de estado según rol, Eliminar Turno).
...
**Bug real corregido — no se podía borrar/editar "Repetir cada" ni "Cantidad de sesiones" (mucho más notorio en mobile)**: ambos inputs numéricos hacían el clamp a su mínimo en cada `onChange`, no solo al perder el foco — `onChange={(event) => setIntervalo(Math.max(1, Number(event.target.value) || 1))}`. Al borrar el dígito (`event.target.value === ''`), `Number('') = 0`, `0 || 1` da `1`, y el input controlado se re-renderizaba instantáneamente con `"1"` — el campo nunca llegaba a mostrarse vacío, así que borrar para escribir un valor nuevo no funcionaba (en desktop, seleccionar todo y sobreescribir en un solo gesto lo disimulaba; en mobile, borrar primero y escribir después es el patrón normal, y ahí quedaba visiblemente "trabado" en 1). Fix: `intervalo`/`cantidad` pasan a admitir `number | ''` — `''` es un estado transitorio válido mientras se escribe (nunca se persiste así: `canConfirm` ya exige `Number.isInteger(...)`, que da `false` para `''`, deshabilitando "Listo" hasta que se complete un valor real) y cada input recupera un valor válido en `onBlur` si se queda vacío o fuera de rango. Verificado con Playwright (viewport mobile): borrar "Repetir cada" → queda vacío (antes: volvía a "1" al instante) → escribir "5" → funciona; borrar y salir del campo sin escribir nada → vuelve a "1" (recuperación esperada, no un valor random). Mismo comportamiento para "Cantidad de sesiones".
...
**Actualización (mobile, ver "Rediseño mobile-first (Home)" en `docs/modules/dashboard.md`)**: en `≤820px` no existe el paso intermedio "card compacta → Más opciones" — se abre directo el formulario completo, y la fila de Repetición (antes exclusiva del modo `compact`) se muestra igual, con label visible en vez de ícono. Esto exigió desacoplar el render de Repetición de `compact` en `FormFields.tsx` (antes `compact && allowRecurrence`, ahora solo `allowRecurrence`, con dos variantes de wrapper — ícono+`sr-only` en compacto, label visible en el resto — sobre el mismo `<select>`/botón/input, sin duplicar controles). En desktop el comportamiento no cambió: Repetición sigue apareciendo solo en la card compacta y desaparece al abrir "Más opciones", igual que antes de esta ronda.
...
**Actualización (implementado) — reutilizado en "Exportar plan de sesiones"**: el PDF de cronograma que se puede exportar desde la tab Turnos del paciente muestra el `numeroSesion` real de cada turno sin recalcularlo — mismo criterio de esta sección ("nunca inventar un número por posición"). Ver "Exportar plan de sesiones (PDF)" en `docs/modules/patients.md` para el detalle completo (selección de próximas sesiones, exclusión de `CANCELADO`, generación del PDF).
