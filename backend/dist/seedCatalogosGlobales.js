...
exports.seedCatalogosGlobales = seedCatalogosGlobales;
...
async function seedCatalogosGlobales() {
    for (let i = 0; i < ESPECIALIDADES_GLOBALES.length; i++) {
        ...
        const existente = await prisma_1.default.especialidad.findFirst({ where: { consultorioId: null, esSistema: true, nombre } });
        if (existente) {
            await prisma_1.default.especialidad.update({ where: { id: existente.id }, data: { color } });
        }
        else {
            await prisma_1.default.especialidad.create({ data: { nombre, color, esSistema: true, consultorioId: null } });
        }
    }
    for (const nombre of OBRAS_SOCIALES_GLOBALES) {
        const existente = await prisma_1.default.obraSocial.findFirst({ where: { consultorioId: null, esSistema: true, nombre } });
        if (!existente) {
            await prisma_1.default.obraSocial.create({ data: { nombre, esSistema: true, consultorioId: null } });
        }
    }
    console.log('Catálogos globales (especialidades, obras sociales) sembrados');
}
//# sourceMappingURL=seedCatalogosGlobales.js.map