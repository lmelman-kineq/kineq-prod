...
exports.withUniqueSuffix = withUniqueSuffix;
exports.issuePresignedUploadUrl = issuePresignedUploadUrl;
...
// store manualmente.
function withUniqueSuffix(nombreOriginal) {
    return `${(0, crypto_1.randomUUID)()}-${nombreOriginal}`;
}
...
function getBlobStoreId() {
    ...
    if (!storeId) {
        throw new Error('BLOB_READ_WRITE_TOKEN_STORE_ID no está configurada — conectá el store de Vercel Blob al proyecto (Storage → conectar a kineq-api) para que Vercel la agregue automáticamente.');
    }
    return storeId;
}
...
// navegador.
async function issuePresignedUploadUrl(pathname, opts) {
    ...
    const signedToken = await (0, blob_1.issueSignedToken)({
        storeId,
        pathname,
        operations: ['put'],
    ...
    const { presignedUrl } = await (0, blob_1.presignUrl)(signedToken, {
        operation: 'put',
        pathname,
        access: 'private',
        allowedContentTypes: opts.allowedContentTypes,
...
async function deleteFromBlob(pathname) {
    try {
        await (0, blob_1.del)(pathname, { storeId: getBlobStoreId() });
    }
    catch (err) {
...
async function streamBlobToResponse(pathname, res, mimeType, downloadName) {
    const result = await (0, blob_1.get)(pathname, { access: 'private', storeId: getBlobStoreId() });
