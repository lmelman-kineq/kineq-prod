...
app.get('/api/turnos', async (req, res) => {
    ...
    const turnos = await prisma_1.default.turno.findMany({ where, include: { paciente: true, profesional: true, especialidad: true, obraSocial: true, grupo: true, serie: true } });
    res.json(turnos);
});
...
app.post('/api/turnos', (0, auth_1.requireRole)(...auth_1.ADMIN_DATA_ROLES), async (req, res) => {
    ...
    try {
        ...
        const turno = await prisma_1.default.turno.create({ data: { consultorioId, pacienteId, profesionalId, especialidadId, obraSocialId: obraSocialId || null, grupoId: resolvedGrupoId, inicio: inicioDate, duracionMinutos: Number(duracionMinutos), numeroSesion: resolvedNumeroSesion, esSesionConsulta: resolvedEsSesionConsulta, monto: monto ?? null, notas: notas || null } });
        const result = await prisma_1.default.turno.findUnique({ where: { id: turno.id }, include: { paciente: true, profesional: true, especialidad: true, obraSocial: true, grupo: true, serie: true } });
...
app.patch('/api/turnos/:turnoId', (0, auth_1.requireRole)(...auth_1.ADMIN_DATA_ROLES), async (req, res) => {
    ...
    try {
        ...
        const result = await prisma_1.default.turno.findUnique({ where: { id: updated.id }, include: { paciente: true, profesional: true, especialidad: true, obraSocial: true, grupo: true, serie: true } });
        res.json(result);
    }
    catch (err) {
...
app.post('/api/turnos/serie', (0, auth_1.requireRole)(...auth_1.ADMIN_DATA_ROLES), async (req, res) => {
    ...
    if (!pacienteId || !profesionalId || !especialidadId)
        return res.status(400).json({ error: 'missing required fields' });
    ...
    for (const raw of fechasInicio) {
        ...
        if (Number.isNaN(d.getTime()))
            return res.status(400).json({ error: 'invalid fechasInicio' });
        inicios.push(d);
    }
    try {
        const paciente = await prisma_1.default.paciente.findFirst({ where: { id: pacienteId, consultorioId, activo: true } });
        ...
        const { serie, turnos } = await prisma_1.default.$transaction(async (tx) => {
            ...
            for (let i = 0; i < inicios.length; i++) {
                ...
                turnos.push(await tx.turno.create({
                    data: {
                        consultorioId,
                        pacienteId,
                        profesionalId,
...
// punto de corte.
async function turnoYSiguientesDeSerie(consultorioId, turnoId) {
    const anchor = await prisma_1.default.turno.findFirst({ where: { id: turnoId, consultorioId, eliminadoAt: null } });
    if (!anchor || !anchor.serieId || anchor.ordenEnSerie === null)
        return null;
    const siguientes = await prisma_1.default.turno.findMany({
        where: { consultorioId, serieId: anchor.serieId, eliminadoAt: null, ordenEnSerie: { gte: anchor.ordenEnSerie } },
        orderBy: { ordenEnSerie: 'asc' },
    });
    const anteriores = await prisma_1.default.turno.count({
...
app.get('/api/turnos/:turnoId/serie', async (req, res) => {
    ...
    if (!found)
        return res.status(404).json({ error: 'turno not found in consultorio or not part of a serie' });
    const siguientesConDatos = await prisma_1.default.turno.findMany({
...
app.patch('/api/turnos/:turnoId/serie', (0, auth_1.requireRole)(...auth_1.ADMIN_DATA_ROLES), async (req, res) => {
    const consultorioId = req.usuario.consultorioId;
    const turnoId = Number(req.params.turnoId);
    if (Number.isNaN(turnoId))
        return res.status(400).json({ error: 'invalid id' });
    try {
        ...
        const esProfesional = req.usuario.rol === client_1.RolUsuario.PROFESIONAL;
        if (esProfesional) {
            const profesionalIdVinculado = await (0, auth_1.requireProfesionalVinculado)(req, res);
            if (profesionalIdVinculado === null)
                return;
        ...
        const inicioPorTurno = new Map();
        for (const o of ocurrencias) {
            const d = new Date(o?.inicio);
            if (!o || typeof o.turnoId !== 'number' || Number.isNaN(d.getTime()))
                return res.status(400).json({ error: 'invalid ocurrencias' });
        ...
        if (payload.duracionMinutos !== undefined) {
            if (!Number.isInteger(payload.duracionMinutos) || payload.duracionMinutos < 15)
                return res.status(400).json({ error: 'invalid duracionMinutos' });
        }
        if (payload.monto !== undefined && payload.monto !== null && (typeof payload.monto !== 'number' || payload.monto < 0)) {
            return res.status(400).json({ error: 'invalid monto' });
        ...
        if (payload.obraSocialId) {
            const obra = await prisma_1.default.obraSocial.findFirst({ where: { id: payload.obraSocialId, consultorioId } });
            if (!obra)
                return res.status(404).json({ error: 'obra social not found in consultorio' });
        }
        if (payload.profesionalId) {
            const prof = await prisma_1.default.profesional.findFirst({ where: { id: payload.profesionalId, consultorioId } });
            if (!prof)
                return res.status(404).json({ error: 'profesional not found in consultorio' });
        }
        if ('grupoId' in payload) {
        ...
        if ('esSesionConsulta' in payload)
            payload.esSesionConsulta = Boolean(payload.esSesionConsulta);
        ...
        const result = await prisma_1.default.$transaction(async (tx) => {
            ...
            for (let i = 0; i < siguientes.length; i++) {
                ...
                actualizados.push(await tx.turno.update({
                    where: { id: t.id },
...
// total que le queda.
app.delete('/api/turnos/:turnoId/serie', (0, auth_1.requireRole)(...auth_1.ADMIN_DATA_ROLES), async (req, res) => {
    const consultorioId = req.usuario.consultorioId;
    const turnoId = Number(req.params.turnoId);
    if (Number.isNaN(turnoId))
        return res.status(400).json({ error: 'invalid id' });
...
app.get('/api/plantillas-evolucion', (0, auth_1.requireRole)(...auth_1.CLINICAL_ROLES), async (req, res) => {
    const consultorioId = req.usuario.consultorioId;
    const plantillas = await prisma_1.default.plantillaEvolucion.findMany({
        where: { consultorioId, activo: true },
        orderBy: { nombre: 'asc' },
    });
    res.json(plantillas);
});
app.post('/api/plantillas-evolucion', (0, auth_1.requireRole)(...auth_1.CLINICAL_ROLES), async (req, res) => {
    const consultorioId = req.usuario.consultorioId;
    const nombre = String(req.body.nombre || '').trim();
    if (!nombre)
        return res.status(400).json({ error: 'nombre es requerido' });
    ...
    const plantilla = await prisma_1.default.plantillaEvolucion.create({
        data: { consultorioId, nombre, contenido, contenidoHtml },
    });
    res.status(201).json(plantilla);
});
app.patch('/api/plantillas-evolucion/:id', (0, auth_1.requireRole)(...auth_1.CLINICAL_ROLES), async (req, res) => {
    const consultorioId = req.usuario.consultorioId;
    const id = Number(req.params.id);
    if (Number.isNaN(id))
        return res.status(400).json({ error: 'invalid id' });
    const plantilla = await prisma_1.default.plantillaEvolucion.findFirst({ where: { id, consultorioId } });
    if (!plantilla)
        return res.status(404).json({ error: 'plantilla not found in consultorio' });
    const payload = {};
    if (req.body.nombre !== undefined) {
        const nombre = String(req.body.nombre).trim();
        if (!nombre)
    ...
    const updated = await prisma_1.default.plantillaEvolucion.update({ where: { id }, data: payload });
    res.json(updated);
});
app.delete('/api/plantillas-evolucion/:id', (0, auth_1.requireRole)(...auth_1.CLINICAL_ROLES), async (req, res) => {
    const consultorioId = req.usuario.consultorioId;
    const id = Number(req.params.id);
    if (Number.isNaN(id))
        return res.status(400).json({ error: 'invalid id' });
    ...
    if (!plantilla)
        return res.status(404).json({ error: 'plantilla not found in consultorio' });
    await prisma_1.default.plantillaEvolucion.update({ where: { id }, data: { activo: false } });
    res.status(204).end();
});
// FICHA INICIAL
