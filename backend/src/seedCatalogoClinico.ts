...
export async function seedCatalogoClinico() {
  const grupos: Array<{ categoria: 'ANTECEDENTE_FAMILIAR' | 'ANTECEDENTE_PERSONAL' | 'PROCEDIMIENTO_QUIRURGICO' | 'ALERGIA'; nombres: string[] }> = [
  ...
  for (const grupo of grupos) {
    for (let orden = 0; orden < grupo.nombres.length; orden++) {
      ...
      if (existente) {
        await prisma.catalogoClinicoItem.update({ where: { id: existente.id }, data: { nombre, orden } })
      } else {
