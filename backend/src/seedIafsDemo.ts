...
function buenosAiresDateParts(date: Date) {
...
async function main() {
  const consultorio = await prisma.consultorio.findFirst({ where: { nombre: CONSULTORIO_NOMBRE } })
  if (!consultorio) {
    throw new Error(
      `No se encontró un consultorio con nombre "${CONSULTORIO_NOMBRE}". Este script no crea el consultorio, solo carga datos demo dentro de uno existente.`,
    )
  }
  ...
  for (const p of profesionalesData) {
    const existing = await prisma.profesional.findFirst({ where: { consultorioId, matricula: p.matricula } })
    const profesional = existing
      ? await prisma.profesional.update({ where: { id: existing.id }, data: p })
      : await prisma.profesional.create({ data: { consultorioId, ...p } })
  ...
  const pacientesData: Array<{ nombre: string; apellido: string; obraSocialId: number | null }> = [
  ...
  for (let i = 0; i < pacientesData.length; i++) {
    ...
    const paciente = existing
      ? await prisma.paciente.update({ where: { id: existing.id }, data })
  ...
  const turnosData: TurnoDemo[] = [
    ...
    { inicio: new Date(now.getTime() + 5 * 60_000), duracion: 30, paciente: 1, profesional: 1, especialidad: 1, estado: EstadoTurno.EN_ESPERA },
    { inicio: atHour(0, 18), duracion: 60, paciente: 2, profesional: 0, especialidad: 1, estado: EstadoTurno.ASIGNADO },

    // Mañana
    { inicio: atHour(1, 9), duracion: 60, paciente: 3, profesional: 0, especialidad: 0, estado: EstadoTurno.ASIGNADO },
    { inicio: atHour(1, 11), duracion: 45, paciente: 4, profesional: 1, especialidad: 2, estado: EstadoTurno.ASIGNADO },
    { inicio: atHour(1, 16), duracion: 60, paciente: 5, profesional: 0, especialidad: 1, estado: EstadoTurno.ASIGNADO },

    // Día +2 (incluye superposición intencional del mismo profesional)
    { inicio: atHour(2, 8), duracion: 60, paciente: 6, profesional: 1, especialidad: 1, estado: EstadoTurno.ASIGNADO },
    { inicio: atHour(2, 10), duracion: 90, paciente: 7, profesional: 0, especialidad: 0, estado: EstadoTurno.ASIGNADO },
    { inicio: atHour(2, 10, 30), duracion: 60, paciente: 0, profesional: 0, especialidad: 1, estado: EstadoTurno.ASIGNADO },

    // Día +3
    { inicio: atHour(3, 14), duracion: 60, paciente: 1, profesional: 1, especialidad: 2, estado: EstadoTurno.ASIGNADO },
    { inicio: atHour(3, 17), duracion: 45, paciente: 2, profesional: 0, especialidad: 0, estado: EstadoTurno.CANCELADO },

    // Día +4
    { inicio: atHour(4, 9), duracion: 60, paciente: 3, profesional: 1, especialidad: 1, estado: EstadoTurno.ASIGNADO },
    { inicio: atHour(4, 13), duracion: 60, paciente: 4, profesional: 0, especialidad: 1, estado: EstadoTurno.ASIGNADO },

    // Día +5
    { inicio: atHour(5, 10), duracion: 45, paciente: 5, profesional: 1, especialidad: 2, estado: EstadoTurno.ASIGNADO },
    { inicio: atHour(5, 18), duracion: 60, paciente: 6, profesional: 0, especialidad: 0, estado: EstadoTurno.ASIGNADO },

    // Día +6
    { inicio: atHour(6, 9, 30), duracion: 60, paciente: 7, profesional: 1, especialidad: 1, estado: EstadoTurno.ASIGNADO },
    { inicio: atHour(6, 15), duracion: 60, paciente: 0, profesional: 0, especialidad: 0, estado: EstadoTurno.ASIGNADO },
  ]
  ...
  for (let i = 0; i < turnosData.length; i++) {
    ...
    const existing = await prisma.turno.findFirst({ where: { consultorioId, notas: clave } })
    if (existing) {
      skipped++
      continue
    }
    ...
    await prisma.turno.create({
      data: {
        ...
        canceladoAt: t.estado === EstadoTurno.CANCELADO ? new Date() : null,
      },
    })
    created++
  }
  ...
  const fichaDemo = await prisma.fichaInicial.upsert({
  ...
  const antecedentesDemo: Array<{
    categoria: 'ANTECEDENTE_PERSONAL' | 'ANTECEDENTE_FAMILIAR' | 'PROCEDIMIENTO_QUIRURGICO'
    codigo: string
    detalle?: string
    parentesco?: string
  }> = [
  ...
  for (const a of antecedentesDemo) {
    const catalogoItem = await prisma.catalogoClinicoItem.findFirst({ where: { categoria: a.categoria, codigo: a.codigo } })
    if (!catalogoItem) continue
    await prisma.fichaAntecedente.upsert({
  ...
  const penicilina = await prisma.catalogoClinicoItem.findFirst({ where: { categoria: 'ALERGIA', codigo: 'penicilina' } })
  if (penicilina) {
    const existingAlergia = await prisma.fichaAlergia.findFirst({ where: { fichaInicialId: fichaDemo.id, catalogoItemId: penicilina.id } })
    if (!existingAlergia) {
      await prisma.fichaAlergia.create({
        data: {
          consultorioId,
          fichaInicialId: fichaDemo.id,
  ...
  for (const m of medicacionDemo) {
    const existing = await prisma.fichaMedicacion.findFirst({ where: { fichaInicialId: fichaDemo.id, nombre: m.nombre } })
    if (existing) continue
    await prisma.fichaMedicacion.create({ data: { consultorioId, fichaInicialId: fichaDemo.id, ...m } })
  }

  // ESTUDIO COMPLEMENTARIO demo (idempotencia por fichaInicialId + tipo).
  const existingEstudio = await prisma.fichaEstudioComplementario.findFirst({ where: { fichaInicialId: fichaDemo.id, tipo: 'Radiografía lumbar' } })
  if (!existingEstudio) {
    await prisma.fichaEstudioComplementario.create({
      data: {
        consultorioId,
        fichaInicialId: fichaDemo.id,
  ...
  for (const e of evolucionesData) {
    ...
    const existing = await prisma.evolucion.findFirst({ where: { consultorioId, pacienteId: pacientes[e.paciente].id, contenido: { startsWith: clave } } })
    if (existing) continue
