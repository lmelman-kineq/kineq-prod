...
import { CategoriaCatalogoClinico } from './generated/prisma/client'

/**
 * Seed base de Kineq para staging/production.
 *
 * Crea únicamente datos globales necesarios para que una instalación vacía funcione.
 * NO crea consultorios, usuarios, profesionales, pacientes, turnos ni evoluciones demo.
 ...
 *   2. Ejecutar las migraciones de producción.
 */

type EspecialidadBase = { nombre: string; color: string }
type ObraSocialBase = { nombre: string }
type CatalogoBase = {
...
const ESPECIALIDADES_BASE: EspecialidadBase[] = [
...
const OBRAS_SOCIALES_BASE: ObraSocialBase[] = [
  { nombre: 'OSDE' },
...
const CATALOGO_CLINICO_BASE: CatalogoBase[] = [
  ...catalogo(CategoriaCatalogoClinico.ANTECEDENTE_PERSONAL, personales),
  ...catalogo(CategoriaCatalogoClinico.ANTECEDENTE_FAMILIAR, familiares),
  ...catalogo(CategoriaCatalogoClinico.PROCEDIMIENTO_QUIRURGICO, quirurgicos),
  ...catalogo(CategoriaCatalogoClinico.ALERGIA, alergias),
]
...
async function seedObrasSociales() {
  ...
  for (const item of OBRAS_SOCIALES_BASE) {
    const existente = await prisma.obraSocial.findFirst({
      where: {
        consultorioId: null,
        esSistema: true,
        nombre: item.nombre,
...
async function seedCatalogoClinico() {
  ...
  for (const item of CATALOGO_CLINICO_BASE) {
    ...
    const existente = await prisma.catalogoClinicoItem.findFirst({
      where: {
        consultorioId: null,
        esSistema: true,
    ...
    if (existente) {
      await prisma.catalogoClinicoItem.update({
        where: { id: existente.id },
        data: {
          nombre: item.nombre,
      ...
      await prisma.catalogoClinicoItem.create({
        data: {
          categoria: item.categoria,
          codigo: item.codigo,
          nombre: item.nombre,
...
async function printSummary() {
  const [especialidades, obrasSociales, personalesCount, familiaresCount, quirurgicosCount, alergiasCount] =
    await Promise.all([
      prisma.especialidad.count({ where: { consultorioId: null, esSistema: true, activo: true } }),
      prisma.obraSocial.count({ where: { consultorioId: null, esSistema: true, activo: true } }),
      prisma.catalogoClinicoItem.count({
        where: {
          consultorioId: null,
          esSistema: true,
          activo: true,
          categoria: CategoriaCatalogoClinico.ANTECEDENTE_PERSONAL,
        },
      }),
      prisma.catalogoClinicoItem.count({
        where: {
          ...
          esSistema: true,
          activo: true,
          categoria: CategoriaCatalogoClinico.ANTECEDENTE_FAMILIAR,
        },
      }),
      prisma.catalogoClinicoItem.count({
        where: {
          ...
          esSistema: true,
          activo: true,
          categoria: CategoriaCatalogoClinico.PROCEDIMIENTO_QUIRURGICO,
        },
      }),
      prisma.catalogoClinicoItem.count({
        where: {
          ...
          esSistema: true,
          activo: true,
          categoria: CategoriaCatalogoClinico.ALERGIA,
        },
      }),
    ])
